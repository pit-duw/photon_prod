...
bool file_exists(const std::string &filename)
{
    ...
    if (stat(filename.c_str(), &sb) == 0)
        return true; // && S_ISREG(sb.st_mode ))
    else
        return false;
}
...
static double Observable[_Ngrids] = {0, 0, 0}; // observable array
int nObsBins[_Ngrids] = {12, 8, 10};           // eta4, pt4 cental eta-bin, pt4 forward eta-bin
...
double _pt[9] = {15, 20, 25, 30, 35, 40, 50, 60, 100};
...
void book_grid() // inital grid booking
{
    ...
    const double *obsBins[_Ngrids] = {_eta, _pt, _ptf};
    ...
    if (basename && std::string(basename) != "")
        glabel = basename;

    const char *q2upper = std::getenv("appl_q2up");
    if (q2upper && std::string(q2upper) != "")
        q2Up = std::atof(q2upper);

    const char *q2lower = std::getenv("appl_q2low");
    if (q2lower && std::string(q2lower) != "")
        q2Low = std::atof(q2lower);

    const char *q2order = std::getenv("appl_q2order");
    if (q2order && std::string(q2order) != "")
        qorder = std::atoi(q2order);
    ...
    {
        for (int ic1 = 0; ic1 < __nf2__; ic1++)
            ckm_vsq[ic + 1][ic1 + 1] = ckm_.vsq[ic][ic1];
    }
...
void fill_grid(const double evt[][mxpart])
{
    ...
    for (int igrid = 0; igrid < Ngrids; igrid++)
        if (cuts(igrid))
        {
            if (evtcounter % 50000 == 0)
            {
                std::cout << "fill_grid() filled " << evtcounter << " weights " << date() << std::endl;
                std::cout << "Observable: " << Observable[igrid] << std::endl;
            }
            evtcounter++;
...
void getObservable(const double evt[][mxpart])
{
    ...
    for (int igrid = 0; igrid < Ngrids; igrid++)
        Observable[igrid] = 0.0; // initialize
...
int cuts(int igrid)
{
    ...
    case (2):
        if ((nproc_.nproc >= 280) && (nproc_.nproc <= 286))
        {
            if (std::fabs(Observable[0]) >= 1.52 && std::fabs(Observable[0]) < 2.37)
                fill = 1;
        }
        else
            fill = 1;
